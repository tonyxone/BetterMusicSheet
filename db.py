    ...
    def create_user_if_missing(user_id, email):
        with _lock:
            _users.setdefault(user_id, {
                "user_id": user_id, "email": email, "display_name": email, "created_at": int(time.time()),
            })

    # ---- user_sub (subscription) ----

    def get_user_sub(user_id):
        item = _user_subs.get(user_id)
        return dict(item) if item else None

    def create_free_sub_if_missing(user_id):
        with _lock:
            _user_subs.setdefault(user_id, {
                "user_id": user_id, "sub_type": "Free", "created_at": int(time.time()),
            })
